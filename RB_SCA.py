"""
Simple Compressible Approximation
...
tstep = pasdetemps      # initial timestep
...
problem.add_equation("dt(T) - (dx(dx(T)) + dz(Tz)) = - u*dx(T) - w*Tz - D/(n+1)*w*T + D/R*diss")    # thermal equation
problem.add_equation("dx(P) - D/R*(dx(dx(u)) + dz(uz)) = 0")    # Stokes equation in the x direction
problem.add_equation("dz(P) - D/R*(dx(dx(w)) + dz(wz)) - D/(n+1)*T = 0")    # Stokes equation in the z direction
...
P['g'] = - Di*(z**2/2.0-1.0/8.0)    # initial hydrostatic pressure
...
initial_dt = tstep
cfl = flow_tools.CFL(solver,initial_dt,safety=0.90, max_change=1.005, min_change=0.2)
...
analysis.add_task("integ(integ(D/R*diss*exp(-Ta),'x'),'z')/(Lxb*Lzb)", name='entrop_source_S_visc')
analysis.add_task("integ(integ(-D/(n+1)*Tz*exp(-Ta),'x'),'z')/(Lxb*Lzb)", name='entrop_source_S_cond')
